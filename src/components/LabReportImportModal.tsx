...
export function LabReportImportModal({
  ...
  return (
    ...
    >
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-sky-100 bg-white p-6 shadow-xl">
        <h2
          id="lab-import-title"
        ...
        {phase === 'scanning' && (
          <p className="mt-6 rounded-xl bg-sky-50 px-4 py-8 text-center text-sm text-slate-600">
        ...
        {phase === 'error' && errorMsg && (
          <div className="mt-6 space-y-3">
            <p className="rounded-xl border border-amber-100 bg-amber-50/80 px-4 py-4 text-sm text-amber-900">
              {errorMsg}
            </p>
            {ocrRawText ? (
              <details className="text-left">
                ...
                <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap break-words rounded-lg border border-slate-100 bg-slate-50 p-3 text-[11px] text-slate-600">
                  {ocrRawText}
                </pre>
              </details>
            ) : null}
        ...
        {phase === 'review' && rows.length > 0 && (
          <div className="mt-6 space-y-3">
              ...
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/80">
                    <th className="px-3 py-2 font-medium text-slate-600">采用</th>
                    <th className="px-3 py-2 font-medium text-slate-600">识别项</th>
                    <th className="px-3 py-2 font-medium text-slate-600">数值</th>
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.key} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="px-3 py-2 text-slate-700">{r.hintLabel}</td>
                      <td className="px-3 py-2">
                        <input
                          type="text"
                          className="w-full min-w-[72px] rounded-lg border border-slate-200 px-2 py-1.5 text-slate-800"
                          value={r.rawValue}
                          onChange={(e) =>
                            setRows((prev) =>
                              prev.map((x) =>
                      ...
                      <td className="px-3 py-2">
                        <select
                          className="w-full max-w-[220px] rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-slate-800"
                          value={r.metricId}
                          onChange={(e) =>
                            setRows((prev) =>
                              prev.map((x) =>
            ...
            {applyHint && (
              <p className="mt-2 text-sm text-amber-800">{applyHint}</p>
            )}
            {ocrRawText ? (
              <details className="mt-4 text-left">
                ...
                <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap break-words rounded-lg border border-slate-100 bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-600">
                  {ocrRawText}
                </pre>
              </details>
            ) : null}
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-3">
          ...
          {phase === 'review' && (
            <button
              ...
              onClick={handleApply}
              className="rounded-xl bg-sky-500 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-sky-600 disabled:cursor-not-allowed disabled:opacity-50"
            >
              填入表单
            </button>
          )}
